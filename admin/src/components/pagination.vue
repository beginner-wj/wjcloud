<template>
     <div class="pagination" role="group" aria-label="分页">
       <button type="button" class="btn btn-default btn-white btn-round" v-bind:disabled="page === 1"
               @click="selectPage(1)">1</button>
       <button type="button" class="btn btn-default btn-white btn-round" v-bind:disabled="page === 1"
               @click="selectPage(page - 1)">上一页</button>
...
<script>
   export default {
     ...
     props:{
       ...
       itemCount: Number //显示的页码数 比如共有100页 只显示10页 其他用省略号
     },
     data: function () {
       return {
         total:0,//总行书
