...
public class LoginAdminGatewayFilter implements GatewayFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        //请求地址中不包括/admin/的 不是控台请求，不需要拦截
        if(!path.contains("/admin/")){
            return chain.filter(exchange);
        }
        if(path.contains("/system/admin/user/login") || path.contains("/system/admin/user/logout")
        ...
        if(object == null){
            LOG.info("token无效,请求被拦截",path);
            exchange.getResponse().setStatusCode(HttpStatus.UNAUTHORIZED);
            return exchange.getResponse().setComplete();
        }
        LOG.info("已登录{}",object);
        LOG.info("接口校验权限,请求地址:{}",path);
        boolean authResult = authRequest(object,path);
        if(!authResult){
            exchange.getResponse().setStatusCode(HttpStatus.UNAUTHORIZED);
            return exchange.getResponse().setComplete();
        }
     ...
     * @param object
     */
    private boolean authRequest(Object object,String path){
        boolean exist = false;
        JSONObject loginUserDto = JSON.parseObject(String.valueOf(object));
        JSONArray requests = loginUserDto.getJSONArray("request");
        //遍历所有【权限请求】,判断当前请求的地址是否在权限请求里
